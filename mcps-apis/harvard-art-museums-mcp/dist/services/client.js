...
const API_KEY_REQUEST_URL = "https://docs.google.com/forms/d/1Fe1H4nOhFkrLpaeBpLAnSrIMYvcAxnYWm0IU9a6IkFA/viewform";
/**
...
export function getApiKey() {
    ...
    if (!key) {
        throw new Error(`HAM_API_KEY is not set. Add it to the .env file in the project root ` +
 ...
 */
export async function hamRequest(endpoint, params = {}) {
...
export function handleApiError(error) {
    if (axios.isAxiosError(error)) {
        ...
        if (axiosError.response) {
            ...
            switch (status) {
                case 400:
                    return ("Error: Bad request (400). A parameter was malformed — most often the 'q' " +
                        "Elasticsearch query string. Simplify the query or check field names.");
                case 401:
                    return ("Error: Unauthorized (401). The API key is missing or invalid. Set a valid " +
                        `HAM_API_KEY in the project's .env file. Request a key at ${API_KEY_REQUEST_URL}`);
                case 404:
                    return ("Error: Not found (404). The id or endpoint does not exist. Verify the id " +
                        "(e.g. object_id) or use a search/list tool to find valid ids first.");
                case 429:
                    return (`Error: Rate limit exceeded (429). The API allows ${DAILY_RATE_LIMIT} requests per ` +
